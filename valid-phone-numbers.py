...
import pandas as pd 
import numpy as np
for i in filter(lambda x : x is not np.nan ,[x if "-" in x else np.nan for x in list(pd.read_csv("deneme.TXT",sep="\n",header=None)[0])]):print(i)

# çözüm:2
import re
wanted_format_1="^[(]{1}[0-9]{3}[)]{1}[\s]{1}[0-9]{3}[-]{1}[0-9]{4}$"  # (123) 456-7890
